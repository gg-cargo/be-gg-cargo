...
export class RatesService {
    ...
    async calculateTruckRentalRate(originLatLng: string, destinationLatLng: string) {
        try {
            let nonTollDistance: number;
            let tollDistance: number;

            // Coba gunakan Mapbox API jika token tersedia
            if (this.mapboxAccessToken) {
                try {
                    // Ambil jarak untuk rute non-tol
                    nonTollDistance = await this.getDistanceFromMapbox(
                        originLatLng,
                        destinationLatLng,
                        true // exclude toll
                    );

                    // Ambil jarak untuk rute tol
                    tollDistance = await this.getDistanceFromMapbox(
                        originLatLng,
                        destinationLatLng,
                        false // include toll
                    );

                    this.logger.log('Successfully retrieved distances from Mapbox API');
                } catch (mapboxError) {
            ...
            const nonTollEstimate = this.calculatePrice(nonTollDistance, false);
            const tollEstimate = this.calculatePrice(tollDistance, true);
            ...
            return {
                ...
                data: {
                    ...
                    estimasi_harga: {
                        non_tol: {
                            jarak_km: Math.round(nonTollDistance * 100) / 100, // Round to 2 decimal places
                            harga_dasar: this.formatRupiah(nonTollEstimate.basePrice),
                            total: this.formatRupiah(nonTollEstimate.totalPrice)
                        },
                        tol: {
                            jarak_km: Math.round(tollDistance * 100) / 100, // Round to 2 decimal places
    ...
    private async getDistanceFromMapbox(
        ...
        try {
            ...
            this.logger.log(`Distance from Mapbox: ${distanceInKm} km (exclude toll: ${excludeToll})`);
